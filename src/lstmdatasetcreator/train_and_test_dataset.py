...
with open("../../output/lstmdataset/trainingdataset.csv", "w", newline='', encoding="utf-8") as train_file:
    writer = csv.writer(train_file)
    writer.writerow(["text", "label"])
    for topic in labels:
        abstracts = training_dataset[topic]
        for i in range(len(abstracts)):
            writer.writerow([abstracts[i], topic])
print("Done ✓")

print("INFO: Writing test file", end="... ")
with open("../../output/lstmdataset/testdataset.csv", "w", newline='', encoding="utf-8") as test_file:
...
with open("../../output/lstmdataset/valdataset.csv", "w", newline='', encoding="utf-8") as val_file:
