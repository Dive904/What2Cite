...
for i in range(len(data_out_citations)):
    ...
    # because, for memory reasons, the two parts of dataset cannot be hold at the same, so we must split the computing
    missing_title = [(paper_id, None) for paper_id in missing]
    for data in dataset:
        # we do in this way, for optimal reason: the dataset is very large, in this way, we read the dataset only once
        try:
    ...
    info_to_write["missing_title"] = missing_title
...
for elem in to_write:
    missing_title = elem["missing_title"]  # get missing title
    ...
    for p in paper_info:  # scannig the second part of the dataset
        if p["id"] in missing_title_none:  # if an id is in the missed title with none
            try:
                ...
                h = None
    elem["missing_title"] = missing_title
print("Done ✓")
